  ...
  pm2.list('', function (err, info) {
    for (let i = 0; i < info.length; i++) {
      ...
      const data = {
        ...
        text: status === 'online' ? 'Online: ' + moment(nfo.pm2_env.pm_uptime).calendar() + ' - MEM: ' + formatBytes(nfo.monit.memory) + ' - CPU: ' + nfo.monit.cpu : status,
    ...
    bus.on('process:*', function (a, b) {
      ...
      pm2.list(e.name, function (err, info) {
        ...
        const data = {
          ...
          sender: 'PM2',
          name: e.name,
          text: status === 'online' ? 'Online: ' + moment(e.pm_uptime).calendar() + ' - MEM: ' + formatBytes(e.monit.memory) + ' - CPU: ' + e.monit.cpu : status,

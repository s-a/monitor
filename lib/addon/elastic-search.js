...
ElasticSearch.prototype.init2 = function () {
  ...
  for (let i = 0; i < ELASTICSEARCH_URIS.length; i++) {
    ...
    const c = client.info(function (err, infos) {
      let info = {}
      if (err) {
        info = err
        ...
        info.text = `${ELASTICSEARCH_URI} server is down!`
        info.status = `${ELASTICSEARCH_URI} server is down!`
      } else {
        info = infos
        info.computername = os.hostname()
        info.sender = 'SYSTEM'
        ...
        info.icon = 'fas fa-database'
        info.text = `${ELASTICSEARCH_URI} server is up`
        info.status = `${ELASTICSEARCH_URI} server is up`
      }

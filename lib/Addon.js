class Addon {
  ...
  env(transporterName, addonName) {
    ...
    const result = (process.env[`${transporterName}_REFRESH_INTERVAL_${addonName}`] || process.env[`DEFAULT_REFRESH_INTERVAL_${addonName}`] || process.env[`DEFAULT_REFRESH_INTERVAL`] || superDefault) * 1000
    return result
  }

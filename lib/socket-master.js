...
function SocketMaster() {
  this.spamControlTimer = setInterval(function () {
    messageHashTable.shift()
  }, 1000 * 60)
  return this
}
...
SocketMaster.prototype.publicBroadcast = function (message) {
  ...
  // const msgHash = hash(message.messageIdentifier)
  const messageWasSendAFewSecondsBefore = (messageHashTable.indexOf(msgHash) !== -1)
  if (true) {
    messageHashTable.push(msgHash)
...
SocketMaster.prototype.start = function (webserverPort, slaveListenerPort) {
  ...
  this.publicWebsocketServer.on('request', function (request) {
    ...
    connection.on('close', function (reasonCode, description) {
      console.log((new Date()) + ' browser Peer ' + connection.remoteAddress + ' disconnected.');
    });

    const initialDataSlots = require(path.join(__dirname, '/../initialDataSlots.json'))
  ...
  this.server.listen(slaveListenerPort, function () {
    console.log((new Date()) + ' Master Server is listening on port ' + (slaveListenerPort));
  });
  ...
  this.wsServer.on('request', (function (request) {
    ...
    connection.on('close', function (reasonCode, description) {
      console.log((new Date()) + ' Slave Peer ' + connection.remoteAddress + ' disconnected.');
    });
...
SocketMaster.prototype.stop = function () {
  clearInterval(this.spamControlTimer)
}

...
function SocketMaster() {
  ...
  const transports = (process.env.TRANSPORT_ADDONS ? (process.env.TRANSPORT_ADDONS || '').split(';') : [])
  for (let t = 0; t < transports.length; t++) {
    const dir = transports[t]
    let transportAddonPath = path.isAbsolute(dir) ? dir : path.join(__dirname, dir)
    transportAddonPath = path.resolve(transportAddonPath)
    const TransportAddon = require(transportAddonPath)
    const transportAddon = new TransportAddon(this)
    transportAddon.init()
    this.transports.push({
      path: transportAddonPath,
      instance: transportAddon
    })
  }
...
SocketMaster.prototype.start = async function (webserverPort, slaveListenerPort) {
  ...
  this.publicWebsocketServer.on('request', function (request) {
    ...
    self.log.info('browser Connection accepted.')

    connection.on('close', function (reasonCode, description) {
      self.log.info('browser Peer ' + connection.remoteAddress + ' disconnected.');
    });

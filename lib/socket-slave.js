...
SocketSlave.prototype.init = async function (addons) {
  ...
  for (let a = 0; a < addons.length; a++) {
    ...
    const addonPath = path.isAbsolute(dir) ? dir : path.join(__dirname, dir)
    ...
    if (addon.report) {
      const rpt = await addon.renderReport('INITIAL')
      this.broadcast(rpt)
    ...
    if (message.type === 'binary') {
      ...
      if (temp.system === 'init') {
        ...
        for (let a = 0; a < self.addons.length; a++) {
          ...
          if (addon.addon.renderReport) {
            let rpt = await addon.addon.renderReport('WEB')
            if (!Array.isArray(rpt)) {
              rpt = [rpt]
            }
            for (let r = 0; r < rpt.length; r++) {
              rpt[r].CRON_EXPRESSION = addon.addon.cronExpression()
              rpt[r].CRON_EXPRESSION_TEXT = addon.addon.humanReadableCronExpression()
            }
            self.broadcast(rpt)
          }
        }
